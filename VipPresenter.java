...
public class VipPresenter {
    ...
    public VipPresenter(VipManager vm, EventManager em){
        vipManager = vm;
        eventManager = em;
    }
    ...
    public String myEventDisplay(){
        ...
        String optionSix = "6: Send a private message to the Speakers of this event.";
    ...
    public String speakersDisplay(String eventName){
        String instruction = "Choose the speaker you want to message. Press enter without typing anything to go back to managing your event.";
        List<String> speakerNames = eventManager.getSpeakersOfEvent(eventName);
        StringBuilder eventList = new StringBuilder();
        for (int i = 1; i <= speakerNames.size(); i++) {
            String num = Integer.toString(i);
            eventList.append(num).append(": ").append(speakerNames.get(i-1)).append("\n");
        }
        return instruction + "\n" + eventList.toString();
    }

    public String toEventMenuDisplay() {return "Returning to managing your event";}

    public String speakerMessageContentDisplay(){ return "Please enter the contents of the message you would like to send.";
    }
